...
if ( ! function_exists( 'dashed_slug_activation_section' ) ) {
	function dashed_slug_activation_section() {
		?><p style="text-size: smaller"><?php
			echo esc_html_e( 'Activate updates to the dashed-slug.net plugins', 'wallets-bitsend' );
		?></p><?php
	}
}
...
if ( ! function_exists( 'dashed_slug_activation_code' ) ) {
	function dashed_slug_activation_code( $args ) {
		?><input name="ds-activation-code" id="ds-activation-code" type="text" value="<?php
			echo esc_attr( call_user_func( is_plugin_active_for_network( 'wallets-bitsend/wallets-bitsend.php' ) ? 'get_site_option' : 'get_option' , 'ds-activation-code' ) );
			?>" />

		<p><?php echo esc_html_e( 'Enter here your dashed-slug.net activation code, exactly as you received it in your e-mail.', 'wallets-bitsend' ); ?></p><?php
	}
}

if ( ! function_exists( 'dashed_slug_notify_missing_code') ) {
	function dashed_slug_notify_missing_code() {
		if ( ! call_user_func( is_plugin_active_for_network( 'wallets-bitsend/wallets-bitsend.php' ) ? 'get_site_option' : 'get_option' , 'ds-activation-code' )) {
	...
	add_action( is_plugin_active_for_network( 'wallets-bitsend/wallets-bitsend.php' ) ? 'network_admin_notices' : 'admin_notices', 'dashed_slug_notify_missing_code' );
}

function wallets_bitsend_get_update_json( $plugin_slug ) {

	$user_nonce = call_user_func( is_plugin_active_for_network( 'wallets-bitsend/wallets-bitsend.php' ) ? 'get_site_option' : 'get_option' , 'ds-activation-code' );
...
if ( ! function_exists( 'wallets_bitsend_pre_set_site_transient_update_plugins' ) ) {
	function wallets_bitsend_pre_set_site_transient_update_plugins( $transient ) {


		$plugin_slug = 'wallets-bitsend';
		$plugin = "$plugin_slug/$plugin_slug.php";

		$update_info = wallets_bitsend_get_update_json( $plugin_slug );
...
add_filter( 'pre_set_site_transient_update_plugins', 'wallets_bitsend_pre_set_site_transient_update_plugins' );

function wallets_bitsend_plugins_api($def, $action, $args) {

	$plugin_slug = 'wallets-bitsend';

	if ( isset( $args->slug) && $args->slug == $plugin_slug && 'plugin_information' == $action ) {
		return wallets_bitsend_get_update_json( $plugin_slug );
	}
...
add_filter('plugins_api', 'wallets_bitsend_plugins_api', 10, 3);
